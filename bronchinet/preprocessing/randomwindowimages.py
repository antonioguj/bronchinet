...
class RandomWindowImages(ImageGenerator):
    ...
    def _compute_gendata(self, **kwargs) -> None:
        seed = kwargs['seed']
        self._crop_window_bounding_box = self._get_random_crop_window_box_image(seed)
    ...
    def _get_random_origin_crop_window_box_image(self, seed: int = None) -> Tuple[int, ...]:
        if seed is not None:
            np.random.seed(seed)

        origin_crop_boundary_box = []
        for i in range(self._ndims):
            searching_space_1d = self._size_volume_image[i] - self._size_image[i]
            origin_1d = np.random.randint(searching_space_1d + 1)
            origin_crop_boundary_box.append(origin_1d)

        return tuple(origin_crop_boundary_box)

    def _get_random_crop_window_box_image(self, seed: int = None) -> BoundBoxNDType:
        origin_crop_boundary_box = self._get_random_origin_crop_window_box_image(seed=seed)
        crop_bounding_box = []
        for i in range(self._ndims):
            limit_left  = origin_crop_boundary_box[i]
            limit_right = origin_crop_boundary_box[i] + self._size_image[i]
    ...
    def get_cropped_image(self, in_image: np.ndarray, seed: int = None) -> np.ndarray:
        crop_bounding_box = self._get_random_crop_window_box_image(seed=seed)
