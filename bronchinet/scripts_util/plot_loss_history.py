...
def main(args):
    ...
    for i, in_file in enumerate(list_input_files):
        ...
        # check whether there are extra fields, and that each has two columns: +1 for validation (with prefix 'val_%')
        ...
        for i, i_field in enumerate(list_extra_fields_novals):
            ...
            if i_valid_field not in list_fields:
                message = 'for the field \'%s\', the validation field \'%s\' not found' % (i_field, i_valid_field)
                catch_error_exception(message)
