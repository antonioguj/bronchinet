...
def main(args):
    ...
    #list_input_files = list_files_dir(args.inputdir, 'CUBE_simulation_Suj01_*left.nii.gz')
    ...
    for i, in_file in enumerate(list_input_files):
        ...
        if args.is_simulated_images:
            if len(in_file_list_splitname) != 7:
                print('WARNING. Expected a filename with 7 strings separated by \'_\'. Found %s... Rename this file by hand'
                      % (len(in_file_list_splitname)))
            ...
            out_file_list_splitname = [in_file_subject + in_file_left_or_right] + [in_file_CUBE_or_GRASE] + [in_file_simulated_or_original] + in_file_twonumbers_first + [in_file_numbers_second]

        else: # normal images / labels
            if len(in_file_list_splitname) != 5:
                print('WARNING. Expected a filename with 5 strings separated by \'_\'. Found %s... Rename this file by hand'
                      % (len(in_file_list_splitname)))
                list_files_failed_rename_by_hand.append(in_file)
                continue
            ...
            out_file_list_splitname = [in_file_subject + in_file_left_or_right] + [in_file_CUBE_or_GRASE] + [in_file_simulated_or_original] + [in_file_numbers_second]
