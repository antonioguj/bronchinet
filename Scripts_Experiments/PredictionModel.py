...
def main(args):
    ...
    nameMasksFiles = 'grndtru*'+ getFileExtension(FORMATTRAINDATA)
    ...
    if (args.cropImages):
        namefile_dict = joinpathnames(BaseDataPath, nameCropBoundingBoxes)
        dict_crop_bounding_boxes = readDictionary(namefile_dict)
    ...
    for ifile, (test_xData_file, test_yData_file) in enumerate(zip(listTestImagesDataFiles, listTestMasksDataFiles)):
        ...
        if (args.constructInputDataDLCST):
            ...
            if (args.slidingWindowImages or args.transformationImages):
                (test_xData, test_yData) = LoadDataManagerInBatches_DataGenerator(IMAGES_DIMS_Z_X_Y,
                                                                                  test_images_generator).loadData_1File(test_xData_file_2,
                                                                                                                                  test_yData_file_2,
                                                                                                                                  shuffle_images=False)
            else:
        ...
        if (args.constructInputDataDLCST):
            ...
            crop_bounding_box = dict_crop_bounding_boxes[filenamenoextension(full_images_file)]

            (bounding_box_left, bounding_box_right) = BoundingBoxes.compute_split_bounding_boxes(crop_bounding_box, axis=2)
        ...
        if (args.masksToRegionInterest):
            ...
            predict_probmaps_array = OperationBinaryMasks.reverse_mask_exclude_voxels_fillzero(predict_probmaps_array,
                                                                                                lungs_masks_array)
            if (args.saveFeatMapsLayers):
                OperationBinaryMasks.reverse_mask_exclude_voxels_fillzero(predict_featmaps_array, lungs_masks_array)
        ...
        if (args.savePredictMaskSlices):
            ...
            General.plot_compare_images_masks_allSlices(full_images_array     [begin_slices:end_slices],
            ...
            if (args.saveFeatMapsLayers):
                for ifeatmap in range(num_save_featmaps):
                    ...
                    General.plot_images_masks_allSlices(full_images_array[begin_slices:end_slices],
