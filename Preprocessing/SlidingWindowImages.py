...
class SlidingWindowImages(BaseImageGenerator):
    ...
    def get_included_image(self, sample_array, images_array, index):
        return NotImplemented

    def get_includedadded_image(self, sample_array, images_array, index):
        return NotImplemented
...
class SlidingWindowImages2D(SlidingWindowImages):
    ...
    def __init__(self, size_image,
        ...
        super(SlidingWindowImages2D, self).__init__(size_image, num_images_total, size_total)

    def complete_init_data(self, in_array_shape):
        self.size_total = in_array_shape[0:self.ndims]
        (self.size_total_x, self.size_total_y) = self.size_total
        (self.num_images_x, self.num_images_y) = self.get_num_images_local()
        self.num_images = self.num_images_x * self.num_images_y

    def get_num_images_dirs(self):
        return (self.num_images_x, self.num_images_y)

    def get_limits_images_dirs(self):
        limits_images_Xdir = [self.get_limits_image_1d(i, self.size_image_x, self.prop_overlap_x, self.size_total_x)
                              for i in range(self.num_images_x)]
        limits_images_Ydir = [self.get_limits_image_1d(i, self.size_image_y, self.prop_overlap_y, self.size_total_y)
                              for i in range(self.num_images_y)]
        return (limits_images_Xdir, limits_images_Ydir)
    ...
    def get_limits_image(self, index):
        ...
        (x_left, x_right) = self.get_limits_image_1d(index_x, self.size_image_x, self.prop_overlap_x, self.size_total_x)
        (y_down, y_up   ) = self.get_limits_image_1d(index_y, self.size_image_y, self.prop_overlap_y, self.size_total_y)
    ...
    def get_included_image(self, sample_array, images_array, index):
        (x_left, x_right, y_down, y_up) = self.get_limits_image(index)
        images_array[x_left:x_right, y_down:y_up, ...] = sample_array
...
class SlidingWindowImages3D(SlidingWindowImages):
    ...
    def __init__(self, size_image,
        ...
        super(SlidingWindowImages3D, self).__init__(size_image, num_images_total, size_total)

    def complete_init_data(self, in_array_shape):
        self.size_total = in_array_shape[0:self.ndims]
        (self.size_total_z, self.size_total_x, self.size_total_y) = self.size_total
        (self.num_images_z, self.num_images_x, self.num_images_y) = self.get_num_images_local()
        self.num_images = self.num_images_x * self.num_images_y * self.num_images_z

    def get_num_images_dirs(self):
        return (self.num_images_z, self.num_images_x, self.num_images_y)
    ...
    def get_limits_images_dirs(self):
        ...
        limits_images_Ydir = [self.get_limits_image_1d(i, self.size_image_y, self.prop_overlap_y, self.size_total_y)
                              for i in range(self.num_images_y)]
        limits_images_Zdir = [self.get_limits_image_1d(i, self.size_image_z, self.prop_overlap_z, self.size_total_z)
                              for i in range(self.num_images_z)]
        return (limits_images_Xdir, limits_images_Ydir, limits_images_Zdir)
    ...
    def get_limits_image(self, index):
        ...
        (x_left, x_right) = self.get_limits_image_1d(index_x, self.size_image_x, self.prop_overlap_x, self.size_total_x)
        (y_down, y_up   ) = self.get_limits_image_1d(index_y, self.size_image_y, self.prop_overlap_y, self.size_total_y)
        (z_back, z_front) = self.get_limits_image_1d(index_z, self.size_image_z, self.prop_overlap_z, self.size_total_z)
    ...
    def get_included_image(self, sample_array, images_array, index):
        (z_back, z_front, x_left, x_right, y_down, y_up) = self.get_limits_image(index)
        images_array[z_back:z_front, x_left:x_right, y_down:y_up, ...] = sample_array
...
class SlicingImages2D(SlidingWindowImages2D):
    def __init__(self, size_image,
                 size_total= (0,0)):
        super(SlicingImages2D, self).__init__(size_image,
                                              (0.0,0.0),
                                              size_total)
class SlicingImages3D(SlidingWindowImages3D):
    def __init__(self, size_image,
                 size_total= (0,0,0)):
        super(SlicingImages3D, self).__init__(size_image,
                                              (0.0,0.0,0.0),
                                              size_total)
