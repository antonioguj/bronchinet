...
def main(args):
    ...
    namesOutputFiles = lambda in_name: basename_file_noext(in_name) + '.nii.gz'
    ...
    if files_extension == '.dcm':
        ...
        tmpsubdir        = join_path_names(args.inputdir, 'tmp')
        makedir(tmpsubdir)
        ...
        list_input_files = list_files_dir(args.inputdir, '*.mhd')
        list_refer_files = list_files_dir(args.inputRefdir)
        prefixPatternInputFiles = get_prefix_pattern_filename(list_refer_files[0])
    ...
    for in_file in list_input_files:
        ...
        if files_type == 'dicom':
            ...
            out_json_file = join_path_names(args.outputdir, basename_file_noext(out_file) + '.json')
            ...
            out_image_array = ImageFileReader.get_image(out_file)
            out_image_array = NiftiReader.fix_dims_image_from_dicom2niix(out_image_array)
            ...
            ImageFileReader.write_image(out_file, out_image_array, metadata=metadata_affine)
            ...
            in_refer_file = find_file_inlist_same_prefix(basename(in_file), list_refer_files,
                                                         prefix_pattern=prefixPatternInputFiles)
            in_metadata   = ImageFileReader.get_image_metadata_info(in_refer_file)
            print("Metadata from file: \'%s\'..." % (basename(in_refer_file)))

            ImageFileReader.write_image(out_file, inout_array, metadata=in_metadata)
    #endfor
