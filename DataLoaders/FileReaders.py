...
class FileReader(object):
    @classmethod
    def get_image_position(cls, filename):
        return cls.get_clsFileReader(filename).get_image_position(filename)

    @classmethod
    def get_image_voxel_size(cls, filename):
        return cls.get_clsFileReader(filename).get_image_voxel_size(filename)

    @classmethod
    def get_image_metadata_info(cls, filename):
        return cls.get_clsFileReader(filename).get_image_metadata_info(filename)

    @classmethod
    def update_image_metadata_info(cls, filename, **kwargs):
        in_metadata = cls.get_image_metadata_info(filename)
        return cls.get_clsFileReader(filename).update_image_metadata_info(in_metadata, **kwargs)

    @classmethod
    def get_image_size(cls, filename):
        return cls.get_image_array(filename).shape

    @classmethod
    def get_image_array(cls, filename):
        return cls.get_clsFileReader(filename).get_image_array(filename)

    @classmethod
    def write_image_array(cls, filename, img_array, **kwargs):
        cls.get_clsFileReader(filename).write_image_array(filename, img_array, **kwargs)

    @staticmethod
    def get_clsFileReader(filename):
    ...
    @staticmethod
    def get_2image_arrays_and_check(filename1, filename2):
        image1_array = FileReader.get_image_array(filename1)
        image2_array = FileReader.get_image_array(filename2)
...
class NIFTIreader(FileReader):
    @staticmethod
    def get_image_affine_matrix(filename):
        return nib.load(filename).affine

    @staticmethod
    def compute_affine_matrix(img_voxelsize, img_position): #, img_rotation):
    ...
    @staticmethod
    def update_affine_matrix(in_affine, img_rescalefactor, img_translatefactor): #, img_rotatefactor):
        # Only consider affine transformations composed on rescaling and translation, for the moment
    ...
    @staticmethod
    def change_dims_affine_matrix(affine):
    ...
    @staticmethod
    def fix_dims_image_array_from_dicom2niix(img_array):
        return np.flip(img_array, 1)

    @staticmethod
    def fix_dims_image_affine_matrix_from_dicom2niix(affine):
        affine[1, 1] = - affine[1, 1]
        affine[1,-1] = - affine[1,-1]
        return affine

    @staticmethod
    def change_dims_image_array_read(img_array):
    ...
    @staticmethod
    def change_dims_image_array_write(img_array):
        # Roll array axis to change dimensions from (dz, dx, dy) to (dx, dy, dz) (nifty format)
    ...
    @classmethod
    def get_image_position(cls, filename):
        affine = cls.get_image_affine_matrix(filename)
    ...
    @classmethod
    def get_image_metadata_info(cls, filename):
        return cls.get_image_affine_matrix(filename)

    @classmethod
    def update_image_metadata_info(cls, in_metadata, **kwargs):
        img_rescalefactor   = kwargs['img_rescalefactor']   if 'img_rescalefactor'   in kwargs.keys() else None
        img_translatefactor = kwargs['img_translatefactor'] if 'img_translatefactor' in kwargs.keys() else None
        return cls.update_affine_matrix(in_metadata, img_rescalefactor, img_translatefactor)

    @classmethod
    def get_image_array(cls, filename, isFix_from_dicom2niix=False):
        nib_img = nib.load(filename)
        return cls.change_dims_image_array_read(nib_img.get_data())

    @classmethod
    def write_image_array(cls, filename, img_array, **kwargs):
        affine = kwargs['metadata'] if 'metadata' in kwargs.keys() else None
        nib_img = nib.Nifti1Image(cls.change_dims_image_array_write(img_array), affine)
...
class DICOMreader(FileReader):
    @staticmethod
    def convert_image_array_Stored_dtype_uint16(img_array):
    ...
    @classmethod
    def write_image_array_Old(cls, filename, img_array, ds_refimg):
