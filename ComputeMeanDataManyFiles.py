...
if( len(sys.argv)!=2 ):
    print("ERROR. Please input the input directory name... EXIT")
    sys.exit(0)
else:
    in_dirname = str(sys.argv[1])
...
list_all_files_indir = findFilesDir(in_dirname, name_input_files)
...
for in_file in list_all_files_indir:
    name_prefix_case = getExtractSubstringPattern(basename(in_file), temp_search_input_files)
...
out_fullfilename = joinpathnames(in_dirname, 'mean_' + '_'.join(basename(list_input_files[1]).split('_')[1:]))
...
for (i, in_file) in enumerate(list_input_files):
    ...
    if i==0:
        ...
        if cases_data_this != cases_data:
            print("ERROR: tests cases found for file '%s' do not coincide with those found previously: '%s'... EXIT" %(data_file, cases_data))
            sys.exit(0)
