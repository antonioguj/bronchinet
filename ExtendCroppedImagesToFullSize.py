...
def main(args):
    ...
    for i, images_file in enumerate(listInputImagesFiles):
        ...
        images_cropped_array = FileReader.getImageArray(images_file)

        print("dimensions of input image: %s..." %(str(images_cropped_array.shape)))

        # 1 step: invert image
        images_cropped_array = FlippingImages.compute(images_cropped_array, axis=0)

        # 2 step: extend image
        images_full_shape = FileReader.getImageSize(refer_images_file)

        images_full_array = ExtendImages.compute3D(images_cropped_array, bounding_box, images_full_shape)

        print("dimensions of full size image: %s..." % (str(images_full_array.shape)))
        ...
        FileReader.writeImageArray(out_images_filename, images_full_array)
    #endfor
