...
class TransformRigidImages(ImageGenerator):
    ...
    def _get_calc_inverse_transformed_image(self, in_array: np.ndarray, is_type_input_image: bool = False) -> np.ndarray:
        raise NotImplementedError
    ...
    def _standardize(self, in_image: np.ndarray) -> np.ndarray:
        ...
        if self._featurewise_center:
            if self._mean is not None:
                ...
                catch_error_exception('This ImageDataGenerator specifies '
...
class TransformRigidImages2D(TransformRigidImages):
    ...
    def _get_calc_inverse_transformed_image(self, in_image: np.ndarray, is_type_input_image: bool = False) -> np.ndarray:
        # Apply: 1st: channel shift inten. / flipping
    ...
    def _apply_transform(in_image: np.ndarray, transform_matrix: np.ndarray,
        ...
        channel_images = [ndi.interpolation.affine_transform(x_channel, final_affine_matrix, final_offset,
                                                             order=1, mode=fill_mode, cval=cval) for x_channel in in_image]
        in_image = np.stack(channel_images, axis=0)
        in_image = np.rollaxis(in_image, 0, channel_axis + 1)
        return in_image
...
class TransformRigidImages3D(TransformRigidImages):
    ...
    def _get_compute_random_transform_matrix(self, seed: int = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        if transform_matrix is not None:
            (d, h, w) = (self._size_image[self._img_dep_axis], self._size_image[self._img_row_axis], self._size_image[self._img_col_axis])
    ...
    def _apply_transform(in_image: np.ndarray, transform_matrix: np.ndarray,
        ...
        channel_images = [ndi.interpolation.affine_transform(x_channel, final_affine_matrix, final_offset,
                                                             order=0, mode=fill_mode, cval=cval) for x_channel in in_image]
        in_image = np.stack(channel_images, axis=0)
        in_image = np.rollaxis(in_image, 0, channel_axis + 1)
        return in_image