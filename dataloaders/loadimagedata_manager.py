...
class LoadImageDataManager(object):
    ...
    @classmethod
    def load_1image(cls, filename: str) -> np.ndarray:
    ...
    @classmethod
    def load_1list_images(cls, list_filenames: List[str]) -> List[np.ndarray]:
        out_list_images = []
        for in_file in list_filenames:
            out_image = cls.load_1image(in_file)
            out_list_images.append(out_image)

        return out_list_images

    @classmethod
    def load_2list_images(cls,
        ...
        for in_file_1, in_file_2 in zip(list_filenames_1, list_filenames_2):
            (out_image_1, out_image_2) = cls.load_2images(in_file_1, in_file_2)
...
class LoadImageDataInBatchesManager(LoadImageDataManager):
    ...
    def load_1image(self,
                    filename: str,
                    max_num_images_batch: int = _max_num_images_batch_default,
                    is_shuffle_data: bool = True
                    ) -> np.ndarray:
        in_stack_images = super(LoadImageDataInBatchesManager, self).load_1image(filename)
    ...
    def load_2images(self,
                     filename_1: str,
                     filename_2: str,
                     max_num_images_batch: int = _max_num_images_batch_default,
                     is_shuffle_data: bool = True
                     ) -> Tuple[np.ndarray, np.ndarray]:
        (in_stack_images_1, in_stack_images_2) = super(LoadImageDataInBatchesManager, self).load_2images(filename_1, filename_2)
    ...
    def load_1list_images(self,
                          list_filenames: List[str],
                          max_num_images_batch: int = _max_num_images_batch_default,
                          is_shuffle_data: bool = True
                          ) -> List[np.ndarray]:
        out_dtype = super(LoadImageDataInBatchesManager, self).load_1image(list_filenames[0]).dtype
        out_batch_images = np.array([], dtype=out_dtype).reshape((0,) + self._size_image)
        sumrun_out_images = 0

        for in_file in list_filenames:
            out_stack_images = super(LoadImageDataInBatchesManager, self).load_1image(in_file)
    ...
    def load_2list_images(self,
        ...
        out_dtype_1 = super(LoadImageDataInBatchesManager, self).load_1image(list_filenames_1[0]).dtype
        out_dtype_2 = super(LoadImageDataInBatchesManager, self).load_1image(list_filenames_2[0]).dtype
        ...
        for in_file_1, in_file_2 in zip(list_filenames_1, list_filenames_2):
            (out_stack_images_1, out_stack_images_2) = super(LoadImageDataInBatchesManager, self).load_2images(in_file_1, in_file_2)
