...
class BatchImageDataGeneratorManyImagesPerLabel(BatchImageDataGenerator2Images):
    ...
    def __init__(self,
        ...
        if len(list_xdata) != num_images_per_label * len(list_ydata):
            message = 'Size of list Xdata \'%s\' not equal to size of list Ydata \'%s\' times the input ' \
                      '\'num_images_per_label\' %s' % (len(list_xdata), len(list_ydata), num_images_per_label)
            catch_error_exception(message)

        super(BatchImageDataGeneratorManyImagesPerLabel, self).__init__(size_image,
                                                                        list_xdata,
                                                                        list_ydata,
                                                                        image_generator,
                                                                        num_channels_in=num_channels_in,
