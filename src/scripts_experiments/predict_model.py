...
from common.constant import BASEDIR, NAME_TESTINGDATA_RELPATH, SIZE_IN_IMAGES, PROP_OVERLAP_SLIDING_WINDOW_TEST, \
    ...
    NAME_REFERENCE_KEYS_POSTERIORS_FILE, IS_FILTER_OUTPUT_NETWORK, PROP_FILTER_OUTPUT_NETWORK, TYPE_DNNLIB_USED
...
def main(args):
    ...
    for ifile, in_image_file in enumerate(list_test_images_files):
        ...
        if args.is_reconstruct_pred_patches:
            ...
            out_prediction_reconstructed = np.squeeze(out_prediction_patches, axis=-1)
...
if __name__ == "__main__":
    ...
    if args.is_test_network_2D:
        ...
        if len(args.size_in_images) != 2:
            message = "Wrong input \'size_in_images\': %s. To test 2D model, input size images must have 2 dims..." \
                      % (args.size_in_images)
            catch_error_exception(message)
    else:
        args.is_test_images_slices = False
        if len(args.size_in_images) != 3:
            message = "Wrong input \'size_in_images\': %s. To test 3D model, input size images must have 3 dims..." \
                      % (args.size_in_images)
            catch_error_exception(message)
