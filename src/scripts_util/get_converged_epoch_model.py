...
def main(args):
    ...
    if args.is_output_move_aver_loss and args.is_move_aver:
        out_aver_losshist_filename = args.input_loss_file.replace('.csv', '') + '_move-aver-%d.csv' % (args.size_aver)
    else:
    ...
    with open(args.input_loss_file, 'r') as infile:
        ...
        data_eval_conver_original = data_fields[:, index_field_eval_conver]
    ...
    data_eval_conver = data_fields[:, index_field_eval_conver]

    print('Evaluate the relative difference of the \'%s\' history, between epochs with patience \'%s\'...'
          % (args.field_eval_conver, args.patience_eval_conver))
    ...
    list_epochs_evaluate = list(epochs[args.patience_eval_conver:])
    ...
    for i_epoch_eval in list_epochs_evaluate:
        ...
        if relerror_diff_values > 0.0:
            if relerror_diff_values < args.thres_converge:
                ...
                epoch_min_value = 1 + np.argmin(data_eval_conver_original[:epoch_converged])
                value_converged = data_eval_conver_original[epoch_converged - 1]
                value_min_found = data_eval_conver_original[epoch_min_value - 1]
            ...
            if relerror_diff_values > args.thres_diverge:
                ...
                value_diverged = data_eval_conver_original[epoch_diverged - 1]
    ...
    if is_found_converge:
        print("\nGOOD: HISTORY OF \'%s\' IS CONVERGED AT EPOCH \'%s\'..." % (args.field_eval_conver, epoch_converged))
    elif is_found_diverge:
        print("\nBAD: HISTORY OF \'%s\' IS DIVERGED AT EPOCH \'%s\'..." % (args.field_eval_conver, epoch_diverged))


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('input_loss_file', type=str)
    parser.add_argument('--field_eval_conver', type=str, default='val_loss')
    parser.add_argument('--patience_eval_conver', type=int, default=20)
    ...
    parser.add_argument('--is_output_move_aver_loss', type=bool, default=False)
